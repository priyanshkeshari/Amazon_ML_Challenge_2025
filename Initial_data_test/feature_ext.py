...
def extract_catalog_features(row):
    ...
    features['Net_Quantity'] = float(value_match.group(1)) if value_match else np.nan 
...
df['Total_Quantity'] = df['Net_Quantity'] * df['Pack_Size']
# Use .where() to prevent division by zero, resulting in NaN where Total_Quantity is 0 or NaN
df['Unit_Price_Calculated'] = df['price'] / df['Total_Quantity'].where(df['Total_Quantity'] != 0)

# Display the final, relevant features
final_features = df[['sample_id', 'price', 'Brand_Name', 'Product_Description', 
                     'Pack_Size', 'Net_Quantity', 'Unit_Type', 'Total_Quantity', 
                     'Unit_Price_Calculated', 'Num_Bullet_Points']]
