# catalog_cleaner_from_catalog_content_gpu.py
import re
import math
import pandas as pd
import spacy
...
# Try to import cuDF, otherwise fall back to pandas for CPU
try:
    import cudf as gpd
    print("cuDF found, enabling GPU processing.")
    GPU_ENABLED = True
except ImportError:
    print("cuDF not found, falling back to CPU processing with pandas.")
    gpd = pd
    GPU_ENABLED = False
...
# spaCy's nlp pipeline can be slow on CPU for large datasets, consider nlp.pipe for efficiency
# For actual GPU acceleration of spaCy, you would need spacy-transformers with a CUDA-enabled PyTorch/TensorFlow backend
# For this script, we'll keep the base spacy.load("en_core_web_sm") for consistency in logic,
# but be aware that the matcher part isn't directly GPU-accelerated by cuDF.
...
# ---------------------------------------------------------------------------
def process_catalog_from_catalog_content(df: gpd.DataFrame, # Use gpd.DataFrame
                                         density_conversion_threshold: float = 0.6) -> gpd.DataFrame: # Use gpd.DataFrame
    """
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc="Processing catalog content"):
    ...
    return gpd.DataFrame(out_rows) # Return gpd.DataFrame

# ---------------------------------------------------------------------------
# Save cleaned catalog CSV
# ---------------------------------------------------------------------------
def save_cleaned_catalog(df: gpd.DataFrame, # Accept gpd.DataFrame
    ...
    df.to_csv(out_path, index=False) # cuDF's to_csv handles this
...
if __name__ == "__main__":
    ...
    # Read CSV using cuDF if GPU is enabled, otherwise pandas
    if GPU_ENABLED:
        df_input = gpd.read_csv(r"C:\Users\acer\Desktop\Programming\ML\Hackathons\Amazon_ML_Challenge_2025\68e8d1d70b66d_student_resource\student_resource\dataset\train.csv", encoding="utf-8", low_memory=False)
    else:
    ...
    # Convert back to pandas for display if GPU is enabled, as pandas display is more universally compatible
    if GPU_ENABLED:
        pd.set_option('display.max_columns', None)
        print(cleaned.to_pandas().to_string(index=False)) # Convert to pandas for printing
    else:
        pd.set_option('display.max_columns', None)
